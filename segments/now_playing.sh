...
TMUX_POWERLINE_SEG_NOW_PLAYING_PLAYERCTL_OPTIONS_DEFAULT=""
...
TMUX_POWERLINE_SEG_NOW_PLAYING_NOT_PLAYING_DEFAULT=""
...
# Options for the playerctl command. see "Description" in playerctl(1).
export TMUX_POWERLINE_SEG_NOW_PLAYING_PLAYERCTL_OPTIONS="${TMUX_POWERLINE_SEG_NOW_PLAYING_PLAYERCTL_OPTIONS_DEFAULT}"
...
export TMUX_POWERLINE_SEG_NOW_PLAYING_NOT_PLAYING="${TMUX_POWERLINE_SEG_NOW_PLAYING_NOT_PLAYING_DEFAULT}"
...
run_segment() {
	...
	# Set display text to now playing or not playing text.
	dt=$np
	if [ -z "$dt" ]; then
		dt="${TMUX_POWERLINE_SEG_NOW_PLAYING_NOT_PLAYING}"
	fi

	if [ -n "$dt" ]; then
		case "$TMUX_POWERLINE_SEG_NOW_PLAYING_TRIM_METHOD" in
		"roll")
			dt=$(tp_roll_text "${dt}" "${TMUX_POWERLINE_SEG_NOW_PLAYING_MAX_LEN}" "${TMUX_POWERLINE_SEG_NOW_PLAYING_ROLL_SPEED_DEFAULT}" "${TMUX_POWERLINE_SEG_NOW_PLAYING_ROLL_MODE}" "${TMUX_POWERLINE_SEG_NOW_PLAYING_ROLL_SEPARATOR}")
			;;
		"trim")
			dt=${dt:0:TMUX_POWERLINE_SEG_NOW_PLAYING_MAX_LEN}
			;;
		esac
		echo "${TMUX_POWERLINE_SEG_NOW_PLAYING_NOTE_CHAR} ${dt}"
	fi
	return 0
}
...
__process_settings() {
	...
	if [ -z "$TMUX_POWERLINE_SEG_NOW_PLAYING_PLAYERCTL_OPTIONS" ]; then
		export TMUX_POWERLINE_SEG_NOW_PLAYING_PLAYERCTL_OPTIONS="${TMUX_POWERLINE_SEG_NOW_PLAYING_PLAYERCTL_OPTIONS_DEFAULT}"
	fi
	if [ -z "$TMUX_POWERLINE_SEG_NOW_PLAYING_RHYTHMBOX_FORMAT" ]; then
		export TMUX_POWERLINE_SEG_NOW_PLAYING_RHYTHMBOX_FORMAT="${TMUX_POWERLINE_SEG_NOW_PLAYING_RHYTHMBOX_FORMAT_DEFAULT}"
	fi
	if [ -z "$TMUX_POWERLINE_SEG_NOW_PLAYING_NOT_PLAYING" ]; then
		export TMUX_POWERLINE_SEG_NOW_PLAYING_NOT_PLAYING="${TMUX_POWERLINE_SEG_NOW_PLAYING_NOT_PLAYING_DEFAULT}"
	fi
}
...
__np_playerctl() {
	if [ "$(playerctl status)" = "Playing" ]; then
		np=$(playerctl ${TMUX_POWERLINE_SEG_NOW_PLAYING_PLAYERCTL_OPTIONS} metadata --format="$TMUX_POWERLINE_SEG_NOW_PLAYING_PLAYERCTL_FORMAT")
		echo "$np"
	fi
}
