...
run_segment() {
	...
	if ! is_tmp_valid "$tmp_file" "$TMUX_POWERLINE_SEG_GITHUB_NOTIFICATIONS_UPDATE_INTERVAL"; then
		read -r -d '' notifications < <(curl -s --url "https://api.github.com/notifications?${api_query}" -D "$tmp_headers_file" --header "Authorization: Bearer $TMUX_POWERLINE_SEG_GITHUB_NOTIFICATIONS_TOKEN")
		if link=$(grep '^link: ' "$tmp_headers_file"); then
			IFS=$',' read -r _ last <<<"$link"
			last_page=$(echo "$last" | sed -r 's/.*page=([0-9]+)>; rel="last"/\1/' | tr -d '\r\n')
			page=2
			until [ "$page" -gt "$last_page" ] || [ "$page" -gt "$TMUX_POWERLINE_SEG_GITHUB_NOTIFICATIONS_MAX_PAGES" ]; do
				read -r -d '' _tmp_notifications < <(curl -s --url "https://api.github.com/notifications?${api_query}&page=$page" --header "Authorization: Bearer $TMUX_POWERLINE_SEG_GITHUB_NOTIFICATIONS_TOKEN")
